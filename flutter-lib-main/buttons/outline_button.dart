...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Flutter OutlinedButton',
      debugShowCheckedModeBanner: false, // Remove debug banner from the app
      theme: ThemeData(
        colorScheme: ColorScheme.fromSeed(seedColor: Colors.deepPurple),
        useMaterial3: true,
      ),
      home: const MyHomePage(title: 'Flutter outlined-button'),
    );
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: ElevatedButton(
            onPressed: () {
              print('clicked elvated button');
            },
            onLongPress: () {
              print('clicking elevated button for too long');
            },
            child: Text('click')));
  }
}
